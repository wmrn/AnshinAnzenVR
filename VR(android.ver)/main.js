...
function init() {
    ...
    navigator.mediaDevices.getUserMedia({
        ...
        .then((devices) => {
            //document.querySelector('pre').textContent += JSON.stringify(devices, null, 2);
            const cameraDevices =
                devices.filter(d => d.kind === 'videoinput');
            const cameraDeviceOnBack =
                cameraDevices.filter(d => d.label.toLowerCase().includes('back'))[0];
            let constraints = {};
            if (cameraDeviceOnBack) {
                constraints.video = {
                    deviceId: {
                        exact: cameraDeviceOnBack.deviceId
                    },
                };
            } else {
                constraints = {
